...
func NewCheckTarget(szUrl, SzType string, readWriteTimeout int) *CheckTarget {
	...
	r11.CustomHeadersRaw = GetCustomHeadersRaw()
	return r11
}
...
func (r1 *CheckTarget) SendOnePayload(str, szPath, szHost string, nTimes int) string {
	_, err := r1.ConnTarget()
	defer r1.Close()
	if nil == err && r1.ConnState {
		for i := 0; i < nTimes; i++ {
			r1.WriteWithFlush(fmt.Sprintf(str, szPath, szHost, r1.CustomHeadersRaw))
		}
		s1 := *r1.ReadAll2Str()
		if "" != s1 {
			return s1
