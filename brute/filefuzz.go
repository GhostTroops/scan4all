...
func reqPage(u string) (*page, *pkg.Response, error) {
	...
	if req, err := pkg.HttpRequset(u, method, "", false, nil); err == nil {
		if pkg.In_slice_int(req.StatusCode, []int{301, 302, 307, 308}) {
			page.is302 = true
		}
...
func FileFuzz(u string, indexStatusCode int, indexContentLength int, indexbody string) (path []string, technologies []string) {
	...
	var page404Title = []string{"404", "不存在", "错误", "403", "禁止访问", "请求含有不合法的参数", "网络防火墙", "访问拦截", "由于安全原因JSP功能默认关闭"}
	var page404Content = []string{"<script>document.getElementById(\"a-link\").click();</script>"}
	...
	if url404, url404req, err := reqPage(u + path404); err == nil {
		...
		if url404.is302 && strings.HasSuffix(url404.locationUrl, url404req.RequestUrl+"/file_not_support/") {
			skip302 = true
		}
	...
	for _, payload := range filedic {
		...
		//ch <- struct{}{}
		//wg.Add(1)
		//go func(payloadx string) {
		if url, req, err := reqPage(u + payload); err == nil {
			if url.is403 && (pkg.In_slice_string(url.title, page403title) || pkg.In_slice_string(req.Body, page403Content)) && !skip403 {
				// 基于403页面文件扫描指纹添加
				switch payload {
				case "/Runtime/Logs/":
					technologies = append(technologies, "ThinkPHP")
			...
			if !pkg.In_slice_int(req.StatusCode, page200CodeList) {
				is404Page = true
			}
			if url.isBackUpPath {
				if !url.isBackUpPage {
			...
			if pkg.In_slice_string(url.title, page404Title) {
				is404Page = true
			}
			if pkg.In_slice_string(req.Body, page404Content) {
				is404Page = true
			}
			...
			if url.is302 {
				...
				if pkg.In_slice_string(req.Location, location404) {
					is404Page = true
				}
				if !strings.Contains(req.Location, req.RequestUrl+payload+"/") {
